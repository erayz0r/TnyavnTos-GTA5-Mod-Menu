...
void GUI::Submenus::Recovery()
{
	...
	GUI::Toggle("Loop", Features::MoneyLoopNC, "1. Enter Nightclub\n2. Open safe & stand close\n3. Start loop", SELECTABLE_DISABLE_SAVE);
	GUI::Break("Unlocks", SELECTABLE_CENTER_TEXT);
	if (GUI::Option("Unlock All", "Unlocks many unlockable GTA Online items", SELECTABLE_DISABLED))
	{
	...
	if (GUI::Option("Player", ""))
	{
		std::string Rank;
		if (Game::keyboard::get_input("Enter desired player rank", Rank, 1, 4) == Game::keyboard::input_status::SUCCESS)
		{
	...
	if (GUI::Option("Crew", ""))
	{
		...
		{
			std::string Rank;
			if (Game::keyboard::get_input("Enter desired crew rank", Rank, 1, 4) == Game::keyboard::input_status::SUCCESS)
			{
